...
let handler = async (m, { conn, args, usedPrefix, command, DevMode }) => {
  ...
  cooldowns[m.sender] = Date.now()
        ...
        if (user.limit >= count * 1) {
            user.limit -= count * 1
            if (Aku > Kamu) {
                conn.reply(m.chat, '`🌵 Veamos que numeros tienen!`\n\n'+ `➠ *Ai Hoshino* : ${Aku}\n➠ *${username}* : ${Kamu}\n\n> ${username}, *PERDISTE* ${formatNumber(count)} ⭐ Estrellas.`.trim(), m)
            } else if (Aku < Kamu) {
                user.limit += count * 2
                conn.reply(m.chat, '`🌵 Veamos que numeros tienen!`\n\n'+ `➠ *Ai Hoshino* : ${Aku}\n➠ *${username}* : ${Kamu}\n\n> ${username}, *GANASTE* ${formatNumber(count * 2)} ⭐ Estrellas.`.trim(), m)
            } else {
                user.limit += count * 1
                conn.reply(m.chat, '`🌵 Veamos que numeros tienen!`\n\n'+ `➠ *Ai Hoshino* : ${Aku}\n➠ *${username}* : ${Kamu}\n\n> ${username} obtienes ${formatNumber(count * 1)} ⭐ Estrellas.`.trim(), m)
            }
        } else conn.reply(m.chat, `No tienes *${formatNumber(count)} ⭐ Estrellas* para apostar!`.trim(), m)
