...
let user = global.db.data.users[m.sender]
  ...
  if (cooldowns[m.sender] && Date.now() - cooldowns[m.sender] < tiempoEspera * 1000) {
    ...
    conn.reply(m.chat, `🚩 Espera ⏱ *${tiempoRestante}* para volver a Trabajar.`, m)
    return
  }
  let resultado = Math.floor(Math.random() * 5000)
  cooldowns[m.sender] = Date.now()
  await conn.reply(m.chat, `🚩 ${pickRandom(works)} *${toNum(resultado)}* ( *${resultado}* ) XP 💫.`, m)
  user.exp += resultado
}
...
const works = [
   ...
   "Reparas las máquinas recreativas y recibes",
    "Dejaste que un negro te rompa el culo y ganas"
] 